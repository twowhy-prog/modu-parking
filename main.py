    ...
    for group in raw.get("data", []):
        for lot in group.get("parkinglots", []):
            ...
            item = {
                ...
                "lat": lat, "lng": lng,
                "ev": lot.get("isEvCharge", False),
                "ev_discount": lot.get("isEvDiscount", False),
            }
            lots.append(item)
...
.tag.ev{{background:rgba(16,185,129,.15);color:#10B981;border:1px solid rgba(16,185,129,.3)}}
.tag.ev-dc{{background:rgba(16,185,129,.1);color:#059669;border:1px solid rgba(16,185,129,.25)}}
  ...
  <div class="grid">
    <div class="panel" id="pp">
      ...
      <div class="fb">
        ...
        <button class="fb-btn" onclick="pF('ev',this)">⚡ EV충전</button>
        <button class="fb-btn" onclick="pF('ev_discount',this)">⚡ EV할인</button>
    ...
    <div class="panel" id="tp">
      ...
      <div class="fb">
        ...
        <button class="fb-btn type-btn" onclick="tF('ev',this)">⚡ 전기차</button>
...
function rP(){{
  ...
  let d=gL().filter(l=>{{if(l.dist>md)return false;if(l.p30==null&&l.p60==null&&l.p120==null&&l.p180==null)return false;if(pFlt==='partner'&&!l.partner)return false;if(pFlt==='cheap'&&(l.p60==null||l.p60>4000))return false;if(pFlt==='ev'&&!l.ev)return false;if(pFlt==='ev_discount'&&!l.ev_discount)return false;if(s&&!l.name.toLowerCase().includes(s))return false;return true;}});
  ...
  tb.innerHTML=d.map(l=>`<tr onclick="openModal('${{l.name.replace(/'/g,"\\'")}}', ${{l.dist}}, ${{l.partner}}, ${{l.p60!==null?l.p60:'null'}}, ${{l.seq}}, ${{l.lat}}, ${{l.lng}})" style="cursor:pointer"><td class="nc">${{l.name}}${{l.partner?' <span class="tag partner">파트너</span>':''}}${{l.ev?' <span class="tag ev">⚡충전</span>':''}}${{l.ev_discount?' <span class="tag ev-dc">⚡할인</span>':''}}</td><td><span class="dist">${{l.dist}}m</span></td><td class="p ${{pc(l.p30)}}">${{fp(l.p30)}}</td><td class="p ${{pc(l.p60)}}">${{fp(l.p60)}}</td><td class="p ${{pc(l.p120)}}">${{fp(l.p120)}}</td><td class="p ${{pc(l.p180)}}">${{fp(l.p180)}}</td></tr>`).join('');
}}
...
function _parseRaw(raw,clat,clng,rm){{
  ...
  for(const g of(raw.data||[]))for(const l of(g.parkinglots||[])){{
    ...
    lots.push({{seq:l.parkinglotSeq,name:l.name,dist:Math.round(d),partner:l.isPartner||false,
      ...
      lat:l.latitude||0,lng:l.longitude||0,ev:l.isEvCharge||false,ev_discount:l.isEvDiscount||false}});
